...
import { EN, enTours, enTour, fmtRub, fmtRangeEn, priceUnitEn } from "@/lib/en";
    ...
    <EnShell altHref={tourUrl(t)}>
      ...
      <article className="wrap pt-10">
        ...
        <div className="mt-8 grid gap-10 lg:grid-cols-[1fr_340px]">
          ...
          <aside className="lg:sticky lg:top-20 lg:self-start"><div className="rounded-xs border border-ice-200 bg-white p-5"><p className="font-display text-3xl font-semibold text-ice-800">{t.priceFrom ? EN.labels.from + " " : ""}{fmtRub(t.priceFrom)}</p><p className="text-sm text-ice-600">{priceUnitEn(t)}</p>{next ? <p className="mt-4 rounded-xs bg-ice-100 px-3 py-2 text-sm"><span className="text-ice-600">{EN.labels.next}</span><br /><b>{fmtRangeEn(next)}</b><br /><span className="font-semibold text-dawn-500">{next.seats} {EN.labels.seats}</span></p> : <p className="mt-4 text-sm text-ice-600">{EN.labels.onRequest}</p>}<div className="mt-5 grid gap-2"><a href="#book" className="btn-dawn">{EN.labels.book}</a><a href={SITE.telegram} className="btn-ghost" target="_blank" rel="noopener">{EN.labels.ask}</a></div></div></aside>
        </div>
      </article>
    </EnShell>
  );
