...
export default function Page() {
  return (<EnShell altHref="/kontakty/"><><JsonLd data={orgJsonLd()} /><section className="wrap pt-10"><h1>{EN.contacts.h1}</h1><p className="mt-4 max-w-2xl text-lg text-ink/80">{EN.contacts.lead}</p>
    <div className="mt-8 grid gap-12 md:grid-cols-2"><dl className="grid gap-4 text-[17px]"><div><dt className="text-sm text-ice-600">Phone / WhatsApp / Telegram</dt><dd><a href={`tel:${SITE.phone2Raw}`} className="text-xl font-semibold no-underline">{SITE.phone2Raw}</a></dd></div><div><dt className="text-sm text-ice-600">Email</dt><dd><a href={`mailto:${SITE.email}`}>{SITE.email}</a></dd></div><div><dt className="text-sm text-ice-600">Telegram / Max</dt><dd className="flex flex-wrap gap-x-4 gap-y-1"><a href={SITE.telegram} target="_blank" rel="noopener">Telegram</a><a href={SITE.max} target="_blank" rel="noopener">Max</a></dd></div><div><dt className="text-sm text-ice-600">Office</dt><dd>{SITE.addressStreetEn}, Irkutsk {SITE.postalCode}, Russia</dd><dd className="mt-1 text-sm text-ink/70">{SITE.hoursEn}</dd></div><div className="text-sm text-ink/70"><dt className="text-ice-600">Company</dt><dd>{SITE.legalName}, Irkutsk, Russia. Federal tour operator registry no. {SITE.rto}.</dd></div></dl>
