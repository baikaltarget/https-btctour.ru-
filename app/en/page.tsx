  ...
  return (
    <EnShell altHref="/">
      ...
      <section className="relative isolate min-h-[80svh] overflow-hidden bg-ice-800 text-white">
        ...
        <div className="absolute inset-0 bg-gradient-to-t from-ice-900/88 via-ice-900/45 to-ice-900/15" aria-hidden="true" />
        <div className="absolute inset-0 bg-gradient-to-r from-ice-900/65 via-ice-900/20 to-transparent" aria-hidden="true" />
      ...
      <section className="wrap py-14"><div className="grid gap-8 border-y border-ice-200 py-10 md:grid-cols-4">{EN.home.trust.map((t, i) => { const icon = ["/img/icons/icon-route.webp", "/img/icons/icon-nature.webp", "/img/icons/icon-water.webp", "/img/icons/icon-route.webp"][i]; return <div key={t.title}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={icon} alt="" aria-hidden="true" className="mb-3 h-10 w-10 opacity-80" /><p className="font-display text-xl font-semibold text-ice-800">{t.title}</p><p className="mt-2 text-[15px] text-ink/75">{t.text}</p></div>; })}</div></section>
