...
import { type Tour, tourUrl, fmtPrice, daysWord, nightsWord, nextDeparture, fmtRange, TAG_LABELS } from "@/lib/content";
  ...
  return (
    <article className="group grid gap-5 border-t border-ice-200 py-6 md:grid-cols-[320px_1fr_auto] md:gap-8 md:py-8">
      ...
      <div className="flex items-start justify-between gap-4 md:flex-col md:items-end md:text-right">
        ...
        {next ? (
          <p className="text-sm text-ink/80"><span className="text-ice-600">Ближайший заезд</span><br />{fmtRange(next)}<br /><span className="text-dawn-500 font-semibold">{next.seats} мест из {next.total}</span></p>
        ) : (
