...
import { EN, fmtRub, fmtRangeEn, priceUnitEn, seatsLabelEn, seatsClassEn, type EnTour } from "@/lib/en";
export default function EnTourRow({ t, e, index = 0 }: { t: Tour; e: EnTour; index?: number }) {
  ...
  return (
    <article className="grid min-w-0 gap-5 border-t border-ice-200 py-6 md:grid-cols-[320px_1fr_auto] md:gap-8">
      <Link href={href} className="relative block aspect-[4/3] overflow-hidden rounded-xs md:aspect-[16/11]" tabIndex={-1} aria-hidden="true"><TourImage src={t.image} alt={`${t.title} — tour photo`} seed={index + 1} sizes="320px" /></Link>
