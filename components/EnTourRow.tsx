...
export default function EnTourRow({ t, e, index = 0 }: { t: Tour; e: EnTour; index?: number }) {
  ...
  return (
    <article className="grid gap-4 border-t border-ice-200 py-6 md:grid-cols-[220px_1fr_auto] md:gap-8">
      <Link href={href} className="relative block aspect-[4/3] overflow-hidden rounded-xs" tabIndex={-1} aria-hidden="true"><TourImage src={t.image} alt="" seed={index + 1} sizes="220px" /></Link>
